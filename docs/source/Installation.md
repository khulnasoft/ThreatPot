...
Note that ThreatPot *needs* a running instance of ElasticSearch of a TPoT to function.
...
If your intention is to publish ThreatPot as a Service you should configure different variables as explained below.
...
The file 'installer_on_tpot.sh' allows the automatic installation of Threatpot on an existing TPot instance.
You can choose the type of Threatpot you want to install (http, https or local).
The installer will either clone Threatpot to '/opt/ThreatPot' or if Threatpot exists on your system you need to input the absolute path to the existing Threatpot folder.
It will prompt you for the necessary information/secrets needed.

Example: `sudo ./installer.sh --type=http --folder=/opt/ThreatPot`

<div class="admonition warning">
<p class="admonition-title">Warning</p>
This installer is not officialy supported neither by Threatpot nor by TPOT maintainers.
It must be considered as a POC to have GB and TPOT installed in the same place.
Threatpot is supported to be executed only in a separate instance and to connect externally with the TPOTs.
</div>