...
## General Guidance
Please refer to [ThreatMatrix Documentation](https://threatmatrix.readthedocs.io/en/latest/) for everything missing here.

## Rules
ThreatPot welcomes contributors from anywhere and from any kind of education or skill level. We strive to create a community of developers that is welcoming, friendly and right.
...
* Before asking questions regarding how the project works, please read *through all the documentation* and [install](https://threatpot.readthedocs.io/en/latest/Installation.html) the project on your own local machine to try it and understand how it basically works. This is a form of respect to the maintainers.
...
To start with the development setup, make sure you go through all the steps in [Installation Guide](https://threatpot.readthedocs.io/en/latest/Installation.html) and properly installed it.
...
```bash
docker exec -it threatpot_uwsgi python manage.py spectacular --file docs/source/schema.yml && make html
```
...
The ThreatPot Frontend is tightly linked to the [`certego-ui`](https://github.com/certego/certego-ui) library. Most of the React components are imported from there. Because of this, it may happen that, during development, you would need to work on that library too.
To install the `certego-ui` library, please take a look to [npm link](https://docs.npmjs.com/cli/v8/commands/npm-link) and remember to start certego-ui without installing peer dependencies (to avoid conflicts with ThreatPot dependencies):
...
The `certego-ui` application comes with an example project that showcases the components that you can re-use and import to other projects, like ThreatPot:
...
##### Run all tests
```commandline
docker exec threatpot_uwsgi python3 manage.py test
```
