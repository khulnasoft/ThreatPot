...
@add_docs(description="Extract Structured IOC Feeds from ThreatPot")
@api_view([GET])
def feeds(request, feed_type, attack_type, age, format_):
    logger.info(
        f"request /api/feeds with params: feed type: {feed_type}, attack_type: {attack_type}, Age: {age}, format: {format_}"
    )
...
def feeds_pagination(request):
    ...
    iocs_queryset = get_queryset(
        request, params["feed_type"], params["attack_type"], params["age"], "json"
    )
...
def get_queryset(request, feed_type, attack_type, age, format_):
    ...
    logger.info(
        f"request from {source}. Feed type: {feed_type}, attack_type: {attack_type}, Age: {age}, format: {format_}"
    )
...
# The Doc does not work as intended. We should refactor this by correctly leveraging DRF
@add_docs(
    description="Request if a specific observable (domain or IP address) has been listed by ThreatPot",
